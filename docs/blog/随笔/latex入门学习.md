...
top: true
cover: true
coverImg: /medias/featureimages/Latex.png
toc: true
aside: true
mathjax: true
categories: Markdown
...
|            \sqrt{ab}             |            $\sqrt{ab}$             |             开平方              |
|           \sqrt[5]{ab}           |           $\sqrt[5]{ab}$           | 开5次根号，根号下多个字符时用{} |
...
$\Bigg( \bigg( \Big( \big((x) \big) \Big) \bigg) \Bigg) $$ \Bigg\{ \bigg\{ \Big\{ \big\{\{x\} \big\} \Big\} \bigg\} \Bigg\}$ {{sensitive}}
...
$$
$\begin{equation}f(x)=3x^{2}+6(x-2)-1 \quad		\tag{1}\end{equation} $		做不到居中，typora中还是得ctrl+shift+m，行间公式且自动带编号（偏好设置里加上）。
...
在 equation 环境中添加 array 环境，就可以实现数组或者表格的形式，其中每个元素用 & 分隔， \hline表示横线。公式中如果有中文，就要用\text{}或者\mbox{}装载，否则不能正常输出中文。
$$
...
\right.$
$$
<img src="./Latex入门学习/矩阵.png" alt="latex公式矩阵表达" style="zoom:60%;" />
$$
\begin{matrix}
a &b &c \\\\
d &e &f \\\\
