...
$$
\sum_{i = 1}^{m} \left\| \sum_{j = 1}^{d'} z_{ij} w_j - x_i \right\|_2^2 = \sum_{i = 1}^{m} {z}_i^\mathrm{T} {z}_i - 2 \sum_{i = 1}^{m} {z}_i^\mathrm{T} {W}^\mathrm{T} {x}_i + \text{const} \propto -\text{tr} \left( {W}^\mathrm{T} \left( \sum_{i = 1}^{m} {x}_i {x}_i^\mathrm{T} \right) {W} \right).
$$
...
$$
-2\sum\limits_{i=1}^mz_i^TW^Tx_i=-2\sum\limits_{i=1}^mx_i^TWW^Tx_i=-2tr(\sum\limits_{i=1}^mx_i^TWW^Tx_i)=-2tr(W^T(\sum\limits_{i=1}^mx_ix_i^T)W)
$$
...
>
> 1. **选择 k 近邻**：保留距离最近的 k 个点作为邻居，其他点的距离设为无穷大（即不直接连接）。
>
